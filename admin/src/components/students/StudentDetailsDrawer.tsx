      ...
      <div className="fixed inset-0 bg-white shadow-lg transform transition-transform duration-300 ease-in-out">
        <div className="h-full flex flex-col">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium">{student.academicQualification}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Desired Degree</p>
                <p className="font-medium">{student.desiredDegree}</p>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-semibold text-green-700">
                  {student.statistics.overallChapterProgress}%
                </p>
                <p className="text-xs text-green-600">
                  {student.statistics.chaptersCompleted}/{student.statistics.totalChapters}
                </p>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <p className="text-sm text-purple-600">Exams Progress</p>
                <p className="text-2xl font-semibold text-purple-700">
                  {student.statistics.overallExamProgress}%
                </p>
                <p className="text-xs text-purple-600">
                  {student.statistics.examsPassed}/{student.statistics.totalExams}
                </p>
              </div>
          ...
          <div className="mb-8">
            ...
            <div className="mb-6">
              <h4 className="text-md font-medium text-gray-700 mb-3">Recent Exam Attempts</h4>
              <div className="space-y-3">
                {student.recentActivity.recentExamAttempts.length > 0 ? (
                  student.recentActivity.recentExamAttempts.map((attempt) => (
                    <div key={attempt.id} className="bg-gray-50 p-3 rounded-lg">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-gray-900">{attempt.examTitle}</p>
                          <p className="text-sm text-gray-500">{attempt.chapterTitle} • {attempt.courseTitle}</p>
                  ...
                  <p className="text-sm text-gray-500">No recent exam attempts</p>
                )}
              </div>
            </div>
            ...
            <div>
              <h4 className="text-md font-medium text-gray-700 mb-3">Recent Video Activity</h4>
              <div className="space-y-3">
                {student.recentActivity.recentVideoProgress.length > 0 ? (
                  student.recentActivity.recentVideoProgress.map((video, index) => (
                    <div key={index} className="bg-gray-50 p-3 rounded-lg">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium text-gray-900">{video.videoTitle}</p>
                          <p className="text-sm text-gray-500">{video.chapterTitle} • {video.courseTitle}</p>
                          <p className="text-xs text-gray-400">
                  ...
                  <p className="text-sm text-gray-500">No recent video activity</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-gray-500">Application Status</p>
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      student.application.status === 'APPROVED' ? 'bg-green-100 text-green-800' :
                      student.application.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{new Date(student.application.appliedAt).toLocaleDateString()}</p>
                  </div>
                  {student.application.reviewedAt && (
                    <div>
                      <p className="text-sm text-gray-500">Reviewed Date</p>
                      <p className="font-medium">{new Date(student.application.reviewedAt).toLocaleDateString()}</p>
                    </div>
                  )}
                  {student.application.reviewedBy && (
                    <div>
                      <p className="text-sm text-gray-500">Reviewed By</p>
                      <p className="font-medium">{student.application.reviewedBy}</p>
                    </div>
                  )}
                  {student.application.rejectionReason && (
                    <div className="col-span-2">
                      <p className="text-sm text-gray-500">Rejection Reason</p>
                      <p className="font-medium text-red-600">{student.application.rejectionReason}</p>
                    </div>
                  )}
                </div>
              </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      {payment.paymentDueDate && (
                        <div>
                          ...
                          <p className="font-medium">{new Date(payment.paymentDueDate).toLocaleDateString()}</p>
                        </div>
                      )}
                    </div>
                  </div>
